...
$(document).on("click", ".enableuser", function (e) {
   ...
   Swal.fire({
      title: 'Are you sure?',
      text: "You want to activate this Staff",
      type: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#068101',
   ...
   "columns": [
      ...
      },
      {
         "data": "action", "render": function (data, type, row, meta) {
            ...
            if (row.status == 1) {
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-info viewuser" style="background-color:;"><abbr title="View User Details"><i class="fas fa-eye">&nbsp;&nbsp;View</i></abbr></button>';
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-success edituser" style="background-color:;"><abbr title="Edit Account"><i class="fas fa-pen">&nbsp;&nbsp;Edit</i></abbr></button>';
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-warning disableuser" style="background-color:;"><abbr title="Disable User"><i class="fas fa-window-close">&nbsp;&nbsp;Deactivate</i></abbr></button>';
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-danger deleteuser" style="background-color:;"><abbr title="Delete Account"><i class="fas fa-trash">&nbsp;&nbsp;Delete</i></abbr></button>';
           } else if (row.status == 2) {
               str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-info viewuser" style="background-color:;"><abbr title="View User Details"><i class="fas fa-eye">&nbsp;&nbsp;View</i></abbr></button>';
               str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-warning enableuser" style="background-color:;"><abbr title="Enable User"><i class="fa fa-check-square">&nbsp;&nbsp;Activate</i></abbr></button>';
               str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-danger deleteuser" style="background-color:;"><abbr title="Delete Account"><i class="fas fa-trash">&nbsp;&nbsp;Delete</i></abbr></button>';
            }
            str += '</div>';
            return str;
