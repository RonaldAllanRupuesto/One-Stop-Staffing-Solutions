...
$(document).on('submit','form#add_user', function(e){
    ...
    $.ajax({
        method:'POST',
        url: base_url + 'updateaccount/addUser',
        data: formData,
        dataType: 'json',
...
$(document).on("click", ".edituser", function(){
    ...
    $.ajax({
        method:'POST',
        url: base_url + 'updateaccount/view_all_users',
        dataType: 'json',
...
$(document).on("submit","#edituser", function(e){
    ...
    $.ajax({
       method: 'POST',
       url: base_url + 'updateaccount/edit_user',
       data: formData,
       processData: false,
       contentType: false,
       cache: false,
...
$(document).on("click", ".disableuser", function (e) {
   ...
   Swal.fire({
      title: 'Are you sure?',
      text: "You want to deactivate this Staff, Once Deactivated, this Staff can no longer have the access to the system",
      type: 'warning',
      ...
      if (result.value) {
         ...
         $.ajax({
            type: 'POST',
            url: base_url + 'updateaccount/disable_user',
            data: { user_id: user_id },
            success: function (data) {
               $(".user_table").DataTable().ajax.reload();
            }
...
$(document).on("click", ".enableuser", function (e) {
   ...
   Swal.fire({
      ...
      if (result.value) {
         ...
         $.ajax({
            type: 'POST',
            url: base_url + 'updateaccount/enable_user',
            data: { user_id: user_id },
            success: function (data) {
               $(".user_table").DataTable().ajax.reload();
            }
...
$(document).on("click", ".deleteuser", function (e) {
   ...
   Swal.fire({
      ...
      if (result.value) {
         ...
         $.ajax({
            type: 'POST',
            url: base_url + 'updateaccount/delete_user',
            data: { user_id: user_id },
            success: function (data) {
               $(".user_table").DataTable().ajax.reload();
            }
...
var user_table = $('.user_table').DataTable({
   ...
   "columns": [
      ...
      },
      {
         "data": "action", "render": function (data, type, row, meta) {
            ...
            if (row.status == 1) {
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-info viewuser" style="background-color:white;"><abbr title="View User Details"><i class="fas fa-eye">&nbsp;&nbsp;View</i></abbr></button>';
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-success edituser" style="background-color:white;"><abbr title="Edit Account"><i class="fas fa-pen">&nbsp;&nbsp;Edit</i></abbr></button>';
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-warning disableuser" style="background-color:white;"><abbr title="Disable User"><i class="fas fa-window-close">&nbsp;&nbsp;Deactivate</i></abbr></button>';
              str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-danger deleteuser" style="background-color:white;"><abbr title="Delete Account"><i class="fas fa-trash">&nbsp;&nbsp;Delete</i></abbr></button>';
           } else if (row.status == 2) {
               str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-info viewuser" style="background-color:white;"><abbr title="View User Details"><i class="fas fa-eye">&nbsp;&nbsp;View</i></abbr></button>';
               str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-warning enableuser" style="background-color:white;"><abbr title="Enable User"><i class="fa fa-check-square">&nbsp;&nbsp;Activate</i></abbr></button>';
               str += '<button data-id="' + row.user_id + '"class="btn btn-sm btn-danger deleteuser" style="background-color:white;"><abbr title="Delete Account"><i class="fas fa-trash">&nbsp;&nbsp;Delete</i></abbr></button>';
            }
            str += '</div>';
            return str;
   ...
   "ajax": {
      "url": base_url + "updateaccount/view_users/",
      "type": "POST"
   },
